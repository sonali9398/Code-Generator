...
def get_gemini_response(question, textContent="", modelName="Gemini", display_only=True):
    try:
        prompt = f"Based on the following content:\n\n{textContent}\n\n{question}" if textContent else question
        response_text = ""
        ...
        for attempt in range(3):  
            try:
                ...
                response_text += f"\n\nResponse generated by {modelName}"
...
def ask():
    ...
    model_name = request.form.get('modelName', 'Gemini')
...
# def get_gpt4_response(question, textContent=""):
#     try:
#         openai.api_key = "your-openai-api-key"  # Replace with your OpenAI API key
        
#         prompt = f"Based on the following content:\n\n{textContent}\n\n{question}" if textContent else question
#         response = openai.Completion.create(
#             engine="gpt-4",  # Specify the model engine
#             prompt=prompt,
...
# # def get_llama_response(question, textContent=""):
#     try:
#         model_name = "meta-llama/Meta-Llama-3.1-8B-Instruct"  # Replace with actual model name
...
#             try:
#                 response = chat.send_message(prompt, stream=True)  # Assuming `chat` is configured for LLaMA
#                 for chunk in response:
#                     if session.get('stop_signal', False):
#                         break
#                     if hasattr(chunk, 'text'):
...
# def get_mistral_response(question, textContent=""):
#     try:
#         prompt = f"Based on the following content:\n\n{textContent}\n\n{question}" if textContent else question
#         response_text = ""
...
#         for attempt in range(3):  # Retry up to 3 times
#             try:
#                 response = chat.send_message(prompt, stream=True)  # Assuming `chat` is configured for Mistral
#                 for chunk in response:
#                     if session.get('stop_signal', False):
#                         break
#                     if hasattr(chunk, 'text'):
...
#     if not query:
#         return jsonify({"result": "No speech detected"}), 400
...
#     # print(f"Response stored with ID {response_id}: {response_text}")  # Debugging line
...
def save_response():
    try:
        ...
        # Construct URLs for viewing the response and generating QR code
        link = url_for('view_response', response_id=response_id, _external=True)
        qr_code = url_for('generate_qr', response_id=response_id, _external=True)

        # Return the link and QR code URL as JSON
        return jsonify({'link': link, 'qr_code': qr_code}), 200
...
def generate_qr(response_id):
    ...
    # return Response(f"<h1>Response:</h1><p>{response_text}</p>", content_type='text/html')
